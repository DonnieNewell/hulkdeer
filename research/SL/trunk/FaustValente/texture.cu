...
__device__ int ConvergeValue(dim3 input_size, int x, int y, DTYPE *output , float epsilon)
{
                          ...
                          , float epsilon)
{
    ...
    converge_value = 0 /* ANY */;
    __syncthreads();
    converge_value_result = ConvergeValue(input_size, x, y, output , epsilon);
    if (0 /* ANY */) {
