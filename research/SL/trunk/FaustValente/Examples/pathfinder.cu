...
extern "C" {
void runPathFinderKernel(dim3 input_size, dim3 stencil_size, int iters, 
                          ...
                          )
{
    ...
    shmem[tx] = value;
    inside = inside && (tx >= stencil_size.x && tx < blockDim.x - stencil_size.x);
    if (!inside) return;
    iter = 0;
    while (true)
    {
        ...
        {
            if (tx >= border.x && tx < blockDim.x - border.x)
                output[uidx] = value;
            break;
        }
        __syncthreads();
 ...
 */
void runPathFinder(DTYPE *host_data, int x_max, int iterations
                    )
{
    ...
    char * KernelName = "runPathFinderKernel";
    ...
    timeInMicroSeconds(twoIterTime, (runPathFinderKernel<<< grid_dims, tile_size >>>(
    ...
    timeInMicroSeconds(oneIterTime, (runPathFinderKernel<<< grid_dims, tile_size >>>(
                                    input_size, stencil_size, 0, device_input, device_output,
                                    pyramid_height, global_ro_data
                                    )));
...
////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int i=1; i<tile_size.x/(2 * stencil_size.x); i++)
    {
        ...
        uint32_t time;
        timeInMicroSeconds(time, (runPathFinderKernel<<< grid_dims, tile_size >>>(
                                      input_size, stencil_size, 0, device_input, device_output,
                                      i, global_ro_data
                                      )));
...
////////////////////////////////////////////////////////////////////////////////////
    ...
    for (int iter = 0; iter < iterations; iter += pyramid_height)
    {
        ...
        runPathFinderKernel<<< grid_dims, tile_size >>>(
 ...
 */
void runPathFinderSetData(DTYPE *host_data, int num_elements)
{
    // TEMPORARY.
