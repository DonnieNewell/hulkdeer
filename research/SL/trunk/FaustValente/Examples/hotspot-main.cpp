...
{
    int grid_rows, grid_cols, iterations, blockSize;
...
#ifdef STATISTICS
    SL_MPI_Init();
    for (int i=500; i<=grid_rows; i+= 500)
    {
        ...
        runHotspot(MatrixTemp, i, i, 300, step_div_Cap, Rx, Ry, Rz);
    }
    SL_MPI_Finalize();
#else
    ...
    runHotspot(MatrixTemp, grid_cols, grid_rows, iterations, step_div_Cap, Rx, Ry, Rz);
