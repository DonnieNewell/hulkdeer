...
int main(int argc, char** argv) {
  int my_rank = -1, x_offset = 0, y_offset = 0, length = 3, grid_height = 1,
          grid_width = 2;
  int id[2] = {0};
  int border[3] = {1, 1, 0};
  int neighbors[26] = {-1};
  ...
  if (0 == my_rank) {
    // initialize block 0
    neighbors[x2DPole1] = 1;
    printf("rank %d exchanges neighbor %s\n", my_rank, neighborString(x2DPole1));
    block = new SubDomain(id, x_offset, length + 2 * border[0], y_offset,
            length + 2 * border[1], grid_height, grid_width, neighbors);
  } else if (1 == my_rank) {
    // initialize block 1
    NeighborTag2D opposite_neighbor = getOppositeNeighbor2D(x2DPole1);
