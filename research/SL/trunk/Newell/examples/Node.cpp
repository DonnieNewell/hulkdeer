...
Node::Node() : weight(0.0), is_CPU(true), balance_count(0) {
}
...
void Node::incrementBalCount() {
  ++balance_count;
}

void Node::decrementBalCount() {
  --balance_count;
}

int Node::getBalCount() const {
...
double Node::getTotalWeight(const int kConfig) const {
  ...
  if (kStrongest == kConfig) {
    ...
    for (size_t i = 0; i < children.size(); ++i) {
      const double kChildWeight = children.at(i).getWeight();
      if (kChildWeight > total)
        total = kChildWeight;
    }
  } else {
    for (size_t child_index = 0; child_index < children.size(); ++child_index) {
      if (children.at(child_index).isCPU())
        cpu_total += children.at(child_index).getTotalWeight(kConfig); // recurse
      else
        gpu_total += children.at(child_index).getTotalWeight(kConfig); // recurse
    }
    total = cpu_total + gpu_total;
    if (kCPUOnly == kConfig)
      total -= gpu_total;
    else if (kGPUOnly == kConfig)
      total -= cpu_total;
  }
